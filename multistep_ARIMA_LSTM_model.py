...
filename = 'procedure_imported_data/TB_ARIMA_LSTM.csv'
...
dataset.columns = ['truth_rate', 'SARIMA_pred', 'LSTM_pred']
dataset.index.name = 'date'
print(dataset.head(5))

# save as processed data file
completed_filename = 'completed_data/completed_TB_ARIMA_LSTM.csv'
...
# data setup
new_filename = 'completed_data/completed_TB_ARIMA_LSTM.csv'
...
history = model.fit(train_X, train_y, epochs=150, batch_size=39, 
...
plt.plot(inv_yhat, linestyle='-', label='ARIMA-LSTM predicted value', marker='o')
plt.plot(inv_y, linestyle='--', label='Observed value', marker='o')

plt.legend()
plt.xlabel('Time series (2016-2017)')
plt.ylabel('Rate')
plt.savefig('3-step ARIMA-LSTM prediction.pdf', dpi=600)
plt.show()
